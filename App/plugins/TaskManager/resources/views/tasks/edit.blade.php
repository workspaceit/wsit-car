@extends('layouts.app')
@section('breadcrumbs', Breadcrumbs::render('tm_tasks.edit', $task))
@section('content')
    ...
    <div class="card" id="app">
        <form method="post" action="{{ route('task-manager.tasks.update', $task) }}" enctype="multipart/form-data">
            @csrf
            @method("PUT")
            <div class="form-group row mr-0 ml-0">
                ...
                <div class="col-lg-3 mt-2">
                    <select name="assigner_id" class="form-control mt-2 mb-2" id="assigner_id">
                        <option value="" selected>@lang('ui.assignee')</option>
                        @foreach ($users as $user)
                            <option value="{{ $user['id'] }}" {{ (old('assigner_id', $task->assigner_id) == $user['id']) ? 'selected' : '' }}>{{ $user['username'] }}</option>
                        @endforeach
                    </select>
                ...
                <label for="priority" class="col-lg-1 col-form-label">Priority</label>
                <div class="col-lg-3">
                    <select name="priority" id="priority" class="form-control mt-2 mb-2 {{ $errors->has('priority') ? 'form-control border-danger' : 'form-control' }}">
                        <option value="0" {{old('priority', $task->priority) === 0 ? 'selected' : ''}}>Urgent</option>
                        <option value="1"  {{old('priority', $task->priority) === 1 ? 'selected' : ''}}>Blocker</option>
                        <option value="2" {{old('priority', $task->priority) === 2 ? 'selected' : ''}}>Major</option>
                        <option value="3" {{old('priority', $task->priority) === 3 ? 'selected' : ''}}>Critical</option>
                        <option value="4" {{old('priority', $task->priority) === 4 ? 'selected' : ''}}>Minor</option>
                        <option value="5" {{old('priority', $task->priority) === 5 ? 'selected' : ''}}>Trivial</option>
                    </select>
                ...
                <label for="delivery_date" class="col-lg-1 col-form-label">Delivery</label>
                <div class="col-lg-3">
                ...
                <label for="reminder" class="col-lg-1 col-form-label mt-2 mb-2">Reminder</label>
                <div class="col-lg-3">
                    <div class="input-group date mt-2" id="reminder" data-target-input="nearest">
                ...
                <label for="links" class="col-lg-1 mt-2 col-form-label mb-2">@lang('task.link')</label>
                <div class="col-lg-3 linked mt-2 mb-2">
                ...
                <div class="col-lg-11">
                    <textarea name="description" id="task-description" rows="3" class="form-control mt-2 mb-2 {{ $errors->has('description') ? 'form-control border-danger' : 'form-control' }}">{{old('description', $task->description)}}</textarea>
    ...
    <script>
            ...
            data: function(){
                return{
                    form: {
                        linked:null,
                        tagClick:false,
                        title: @json(old("title", $task->title))
                    }
                }
            },
        ...
        $(document).ready(function() {
            ...
            if(@json(!empty(old("delivery_date", $task->delivery_date)))){
                document.getElementById("delivery_date").defaultValue = @json(old("delivery_date", $task->delivery_date));
            }

            if(@json(!empty(old("reminder", $task->reminder)))){
                document.getElementById("reminder").defaultValue = @json(old("reminder", $task->reminder));
            }
            ...
            $('#delivery_date').datetimepicker({
                defaultDate: @json(old('delivery_date', $task->delivery_date)),
            ...
            let fileinput_options = {
                ...
                maxFileCount: 40,
                maxFileSize: 12288,
                initialPreview: @json($task->files),
                initialPreviewConfig: @json($task->config),
                deleteExtraData: {
                    _token: "{{ csrf_token() }}",
                    task: @json($task->id),
                },
